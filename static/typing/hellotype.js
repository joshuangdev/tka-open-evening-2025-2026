document.addEventListener('DOMContentLoaded', function() {
  ...
  function parseText(text = testText, wordCount = 13) {
  ...
  function fetchTest(test, wordCount) {
    ...
    fetch(`/typing/generate?collection=${test}&word_count=${wordCount}`)
      .then(response => response.json())
      .then(data => {
        // Corrected variable name from 'textText' to 'testText'
        testText = parseText(data.text, wordCount || 13);
  ...
  if (startBtn) startBtn.addEventListener('click', function() {
    reset();
    if (inputEl) inputEl.value = "";
    fetchTest(test, wordCountEl ? wordCountEl.value : 13);
    ongoing = true;
